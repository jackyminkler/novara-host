...
**Single source of truth.** Version 1.7.1, updated 2026-09-01.
...
| **proposedRuns lifecycle** | `matching/pipeline.js`, `expireRunRequests`, `lib/pace_match/` | **Shipped 2026-08-26, merged 2026-09-01** | Three gaps closed. The feed bounds `proposedDate` to today through +7d and applies MATCH-07's app-side expiry to the stream. The hourly sweep collection-group-flips stale actives to `expired` (new composite index, deploy `--only firestore` with it); claimed docs are exempt, and docs are flipped rather than deleted so the deferred TTL still has its purge field. Generation keys every pair on the sorted uid pair and skips it if already written this pass, if either side holds a live active proposal, or if a pending request exists in either direction. A declined or expired request does not block: silent decline has no cool-down |
| **Confirm-sheet claim race** | `lib/pace_match/confirm_run_sheet.dart` | **Shipped 2026-08-27, merged 2026-09-01** | `_handleSend` re-fetches the proposedRun immediately before writing the request and refuses unless it is live. The sheet used to write off its open-time snapshot, so a partner claiming the pairing mid-sheet still received a crossed request. The `claimed_by_other_side` path now closes into the incoming-request screen with `justClaimed`, making the S3 mirror-claim banner reachable for the first time. Blocked sends log `match_request_blocked` with a reason. **Residual, accepted:** the window narrows from minutes to one round trip, and two truly simultaneous sends can still cross, because the mirror-claim function is reactive rather than a gate |
...
| 20 | **Two guards that each trust the other's bookkeeping can wedge a record permanently.** When one mechanism skips work because a second will clean up after it, the first must re-derive the condition rather than trust the flag the second maintains. | Pairing dedupe skips any pair holding an *active* proposal, and an hourly sweep is what flips stale actives to expired. Had the sweep ever lagged, or had the dedupe trusted `status` alone, one stale document would have suppressed its pairing forever and nothing would have reported it. The dedupe therefore re-checks `expiresAt` itself and reads a missing value as never-expiring, which is the one reading consistent with both the feed (which shows it) and the sweep (which cannot match it) | **Closed by design 2026-08-26, merged 2026-09-01.** Behavioural cases drive the real `generateForUsers` and `expireRunRequests` call sites in `proposed_runs_lifecycle.test.js` rather than reconstructions, per trap 16 |
